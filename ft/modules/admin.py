...
from ft import CMD_HELP, DEVS, WHITELIST
...
from ft.utils.logger import logging
...
async def spider(spdr):
    try:
        from ft.modules.sql_helper.spam_mute_sql import mute
    ...
    if user.id in WHITELIST:
        return await man.edit("**Gagal Mute, dia adalah admin @Sharingft 🤪**")
    await man.edit(
        r"\\**#Muted_User**//"
        f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})\n"
        f"**User ID:** `{user.id}`\n"
...
async def unmoot(unmot):
    ...
    try:
        from ft.modules.sql_helper.spam_mute_sql import unmute
    except AttributeError:
...
async def muter(moot):
    try:
        ...
        from ft.modules.sql_helper.spam_mute_sql import is_muted
    except AttributeError:
        return
