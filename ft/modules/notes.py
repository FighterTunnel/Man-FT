...
#
""" ft module containing commands for keeping notes. """
...
async def notes_active(svd):
    try:
        from ft.modules.sql_helper.notes_sql import get_notes
...
async def remove_notes(clr):
    ...
    try:
        from ft.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
async def add_note(fltr):
    try:
        from ft.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).getnt.client:
            try:
                from ft.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
