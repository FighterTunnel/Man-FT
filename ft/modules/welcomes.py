...
from ft import BLACKLIST_CHAT, BOTLOG_CHATID, CLEAN_WELCOME
from ft import CMD_HANDLER as cmd
from ft import CMD_HELP, LOGS
from ft.utils import chataction, man_cmd


@chataction()
async def welcome_to_chat(event):
    try:
        from ft.modules.sql_helper.welcome_sql import (
...
async def save_welcome(event):
    ...
    try:
        from ft.modules.sql_helper.welcome_sql import add_welcome_setting
    except AttributeError:
...
async def show_welcome(event):
    try:
        from ft.modules.sql_helper.welcome_sql import get_current_welcome_settings
    except AttributeError:
...
async def del_welcome(event):
    try:
        from ft.modules.sql_helper.welcome_sql import rm_welcome_setting
    except AttributeError:
        return await event.edit("`Running on Non-SQL mode!`")
