...
#
"""
This module updates the ft based on upstream revision
"""
...
from ft import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME
    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(
                ...
                " **untuk deploy perubahan terbaru dari ft.**",
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await edit_or_reply(
                xx,
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy Man-ft dyno.**",
            )
            return repo.__del__()
        try:
            from ft.modules.sql_helper.globals import addgvar, delgvar
        ...
        await edit_or_reply(
            xx, "`Man-ft Berhasil Di Deploy! ft bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`Man-ft Berhasil Diupdate! ft bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    try:
        ...
        if conf is None:
            return await xx.edit(
                f"**Sayangnya, Directory {error} Tampaknya Bukan Dari Repo. Tapi Kita Bisa Memperbarui Paksa ft Menggunakan** `{cmd}update deploy`"
            )
        repo = Repo.init()
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy Man-ft Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**✥ Man-ft Sudah Versi Terbaru**")
        return repo.__del__()
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**Ketik** `{cmd}update deploy` **untuk Mengupdate ft.**"
        )

    if force_update:
        await xx.edit("**Sinkronisasi Paksa Ke Kode ft Terbaru, Harap Tunggu...**")
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru Man-ft.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari Man-ft.\
    "
    }
)
