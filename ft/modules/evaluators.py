...
#
"""ft module for executing code and terminal commands from Telegram."""
...
async def _(event):
    ...
    if expression in ("ft.session", "config.env"):
...
async def run(event):
    ...
    if code in ("ft.session", "config.env"):
        return await event.edit("`Itu operasi yang berbahaya! Tidak diperbolehkan!`")
...
async def terminal_runner(event):
    ...
    if command in ("ft.session", "config.env"):
        return await event.edit("`Itu operasi yang berbahaya! Tidak diperbolehkan!`")
    await event.edit("`Processing...`")
