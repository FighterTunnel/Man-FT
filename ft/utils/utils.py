...
async def autopilot():
    LOGS.info("TUNGGU SEBENTAR. SEDANG MEMBUAT GROUP LOG FT UNTUK ANDA")
    desc = "Group Log untuk Man-FT.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @fightertunnell ✨"
    try:
        grup = await bot(
            CreateChannelRequest(title="Log FT", about=desc, megagroup=True)
        )
...
async def autobot():
    ...
    if isdone.startswith("Sorry,"):
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "ft/resources/logo.jpg")
            await asyncio.sleep(3)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ft/modules/{shortname}.py")
        name = f"ft.modules.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"ft.modules.{shortname}"] = mod
        LOGS.info(f"Successfully imported {shortname}")
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ft/modules/assistant/{shortname}.py")
        name = f"ft.modules.assistant.{shortname}"
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules[f"ft.modules.assistant{shortname}"] = mod
